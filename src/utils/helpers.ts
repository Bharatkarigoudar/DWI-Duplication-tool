...
export function extractSuffixFromName(
  ...
  try {
    ...
    // Find where {n} sits in the SUBSTITUTED pattern. (Using the raw `pattern`
    // index here was a bug: replacing the 11-char "{base_name}" token with a
    // base name of a different length shifts the position of "{n}", so the
    // prefix/suffix slices below only lined up when the base name happened to
    // be exactly 11 characters long — otherwise extraction returned null and
    // child-suffix inheritance silently did nothing.)
    const nIndex = patternWithBase.indexOf('{n}');
...
export function generateChildName(
  ...
  if (!applyInheritance || !parentSuffix) {
    return originalName;
  }

  // Build "<name> [prefix] <suffix>" with single spaces, so an empty prefix
  // still yields "Task A 001" (previously it produced "Task A001" / "Task A Lot001").
  const parts = [originalName.trim()];
  const trimmedPrefix = prefix.trim();
  if (trimmedPrefix) parts.push(trimmedPrefix);
  parts.push(parentSuffix);
